...
dotNetBuild.AddTarget("updateVersionNumber", "Update version number", c 
    => c.Do(context => {
            ...
            var assemblyInfoTask = new AssemblyInfo
            {
                ...
                {
                    Path.Combine(solutionDirectory, @"SignalR.Castle.Windsor\Properties\AssemblyInfo.cs")
                },
...
dotNetBuild.AddTarget("createPackage", "Create NuGet package", c 
    => c.Do(context => {
            ...
            var nugetPackTask = new Pack
            {
                ...
                NuSpecFile = Path.Combine(solutionDirectory, @"SignalR.Castle.Windsor\package.nuspec"),
...
dotNetBuild.AddTarget("publishPackage", "Publish NuGet package", c
	=> c.Do(context => {
            ...
            var nupkgFile = string.Format(@"packagesForNuGet\SignalR.Castle.Windsor.{0}.nupkg", context.ParameterProvider.Get("VersionNumber"));
